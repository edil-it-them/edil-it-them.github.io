<script lang="ts">
	...
	export let tech: string | undefined = undefined;
	export let awards: string | undefined = undefined;
	export let outro: string | undefined = undefined;
...
<Slide {theme}>
	<div class="slide-content intro">
		...
		{#if +level < 11}
			<p class="next">
				{+level == 10 ? 'Other projects' : 'Project ' + (+level + 1)}
...
{#each slides as { text, className, chart, image, theme }, i}
	<Slide {theme}>
		<div class="slide-content {className || ''}">
			...
			{:else if +level == 1 && i === slides.length - 1}
				<p class="prev">
					My Stack
					<span> <Icon name="arrow-up" /></span>
				</p>
			{:else if +level == 0 && i === slides.length - 1}
				<p class="prev">
					Intro
					<span> <Icon name="arrow-up" /></span>
				</p>
